...
function _Selection (options = {}) {
  const that = {
    ...
    _onTapStop (evt, noevent) {
      ...
      // Reset scroll speed
      that._scrollSpeed.x = 0;
      that._scrollSpeed.y = 0;
    ...
    _updatedTouchingElements () {
      ...
      for (let i = 0; i < _selectables.length; i++) {
        ...
        if ((mode && {}.toString.call(mode) === '[object Function]') ? mode(_areaDomRect, node) : intersects(_areaDomRect, node.getBoundingClientRect(), mode)) {
...
export const Selection = {
  ...
  /**
     ...
     */
  create: options => _Selection(options)
};
